...
function handleSubscribeRows(rows, unsubscribes, buses, filter, callback, errorCallback) {
  rows.reduce((acc, subscribeRow) => {
    if (subscribeRow.path) {
      handleSubscribeRow(subscribeRow, unsubscribes, buses, filter, callback, errorCallback);
    }
    return acc
  }, unsubscribes)
}

function handleSubscribeRow(subscribeRow, unsubscribes, buses, filter, callback, errorCallback) {
...
function contextMatcher(selfContext, subscribeCommand) {
  ...
  if (subscribeCommand.context) {
    return normalizedDeltaData => matcher.test(normalizedDeltaData.context) || ((subscribeCommand.context === 'vessels.self' || subscribeCommand.context === 'self') && normalizedDeltaData.context === selfContext)
  }
  return x => true
}
